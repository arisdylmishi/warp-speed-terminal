...
<style>
    ...
    .error-box { background-color: #330000; color: #ffcccc; padding: 10px; border-left: 5px solid red; }
</style>
...
if not st.session_state['logged_in']:
    ...
    try: st.image("dashboard.jpg", caption="System Preview", use_column_width=True)
    except: st.warning("dashboard.jpg not found")
...
# --- B. MAIN TERMINAL ---
    ...
    except: st.error("Macro Data Offline")
    ...
    if submitted:
        ...
        if not tickers:
            ...
            for i, t in enumerate(tickers):
                try:
                    # 1. Fetch History (Safest Method)
                    stock = yf.Ticker(t)
                    df = stock.history(period="1y")
                    
                    # 2. Check Data
                    if df.empty:
                        st.markdown(f"<div class='error-box'>❌ {t}: No data found. Check spelling.</div>", unsafe_allow_html=True)
                        continue
                    ...
                    st.markdown(f"<div class='error-box'>⚠️ Error scanning {t}: {str(e)}</div>", unsafe_allow_html=True)
    ...
    if 'scan_results' in st.session_state and st.session_state['scan_results']:
        ...
        selected_ticker = st.selectbox("Select Asset to Analyze", [r['TICKER'] for r in results])
        ...
        if target:
            ...
            with t2:
                ...
                c3.metric("Recommendation", info.get('recommendationKey', 'N/A').upper())
